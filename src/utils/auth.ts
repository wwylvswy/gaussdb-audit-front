...
/** 将后端返回的 expires 字符串转为时间戳 */
function parseExpires(expiresStr: string): number {
  // 示例："2025/08/20 23:45:26"
  return new Date(expiresStr).getTime();
}
...
//       : {}
//   );
//   function setUserKey({ avatar, account, nickname, roles, permissions }) {
...
export function setToken(data: DataInfo<string>) {
  const expires = parseExpires(data.expires);
  const { accessToken, refreshToken } = data;
  const { isRemembered, loginDay } = useUserStoreHook();
  const cookieString = JSON.stringify({ accessToken, expires, refreshToken });
